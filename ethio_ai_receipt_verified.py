...
async def show_payment_instructions(
    ...
    sms_callback = "verify_sms_monthly" if plan == "monthly" else "verify_sms_yearly"
    keyboard = [
        ...
        [InlineKeyboardButton("📩 Verify Telebirr SMS", callback_data=sms_callback)],
        [InlineKeyboardButton("⬅️ Back", callback_data="premium_menu")],
    ]
...
async def button_handler(
    ...
    if data == "verify_sms_monthly":
        context.user_data["awaiting_payment_sms"] = "monthly"
        await query.message.reply_text(
            "📩 Copy the FULL Telebirr SMS from 127 and paste it here.\n\n"
            f"Required amount: {MONTHLY_PRICE_ETB:.2f} ETB"
        )
        return

    if data == "verify_sms_yearly":
        context.user_data["awaiting_payment_sms"] = "yearly"
        await query.message.reply_text(
            "📩 Copy the FULL Telebirr SMS from 127 and paste it here.\n\n"
            f"Required amount: {YEARLY_PRICE_ETB:.2f} ETB"
        )
        return
...
# =========================================================
# TELEBIRR SMS / RECEIPT VERIFICATION
# =========================================================
...
def _money_to_float(value):
    if value is None:
        return None
    m = re.search(r"([0-9][0-9,]*(?:\.\d+)?)", str(value))
    if not m:
        return None
    try:
...
def extract_telebirr_sms(sms_text):
    """Extract the reference, receipt URL, amount, receiver and date from a Telebirr SMS."""
    text = " ".join((sms_text or "").split())

    url_match = re.search(
        r"https://transactioninfo\.ethiotelecom\.et/receipt/([A-Za-z0-9]{6,20})",
        text,
        re.I,
    )
    reference = url_match.group(1).upper() if url_match else None
    receipt_url = (
        f"https://{TELEBIRR_RECEIPT_HOST}/receipt/{reference}"
        if reference
        else None
    )

    tx_match = re.search(
        r"transaction\s+number\s+is\s+([A-Za-z0-9]{6,20})",
        text,
        re.I,
    )
    if not reference and tx_match:
        reference = tx_match.group(1).upper()
        receipt_url = f"https://{TELEBIRR_RECEIPT_HOST}/receipt/{reference}"

    amount = None
    amount_match = re.search(
        r"transferred\s+ETB\s*([0-9][0-9,]*(?:\.\d+)?)\s+to\s+(.+?)\s*\(",
        text,
        re.I,
    )
    receiver = None
    if amount_match:
        amount = _money_to_float(amount_match.group(1))
        receiver = amount_match.group(2).strip()
...
def fetch_telebirr_receipt(reference):
    if not reference or not re.fullmatch(r"[A-Za-z0-9]{6,20}", reference):
        raise ValueError("Invalid Telebirr transaction reference.")

    url = f"https://{TELEBIRR_RECEIPT_HOST}/receipt/{reference.upper()}"
    response = requests.get(
        url,
        headers={
            "User-Agent": "Ethio-AI-Payment-Verifier/1.0",
    ...
    if not response.text.strip():
        raise RuntimeError("Empty Telebirr receipt response.")
    return url, response.text


def parse_telebirr_receipt(reference, raw_html):
    """Best-effort parser for the official bilingual Telebirr receipt page."""
    ...
    receipt_no = first([
        r"(?:Receipt\s*No\.?|የደረሰኝ\s*ቁጥር)\s*[:\-]?\s*([A-Za-z0-9]+)",
        r"(?:Transaction\s*(?:Number|No\.?|ID))\s*[:\-]?\s*([A-Za-z0-9]+)",
    ]) or reference.upper()

    status = first([
        r"(?:Transaction\s*Status|transaction status|የክፍያው\s*ሁኔታ)\s*[:\-]?\s*([^|;]+?)\s+(?:Receipt|Payment|Amount|የ)",
    ])

    credited = first([
        r"(?:Credited\s*Party\s*Name|credited party name|የገንዘብ\s*ተቀባይ\s*ስም)\s*[:\-]?\s*(.+?)\s+(?:Credited|Transaction|Payment|Amount|Receipt|የ)",
    ])

    payer = first([
        r"(?:Payer\s*Name|payer name|የከፋይ\s*ስም)\s*[:\-]?\s*(.+?)\s+(?:Payer|Credited|Transaction|Payment|Amount|Receipt|የ)",
    ])

    settled_raw = first([
        r"(?:Settled\s*Amount|settled amount|የተከፈለ\s*መጠን)\s*[:\-]?\s*([0-9][0-9,]*(?:\.\d+)?)\s*(?:Birr|ETB)?",
    ])

    total_raw = first([
        r"(?:Total\s*Paid\s*Amount|total paid amount|ጠቅላላ\s*የተከፈለ)\s*[:\-]?\s*([0-9][0-9,]*(?:\.\d+)?)\s*(?:Birr|ETB)?",
    ])

    payment_date = first([
        r"(?:Payment\s*Date|payment date|የክፍያ\s*ቀን)\s*[:\-]?\s*([^|;]+?)\s+(?:Settled|Service|Total|Payment|Transaction|Amount|የ)",
    ])
    ...
    return {
        ...
        "credited_party_name": credited,
        "payer_name": payer,
        "settled_amount": _money_to_float(settled_raw),
        "total_paid_amount": _money_to_float(total_raw),
        "payment_date": payment_date,
        "raw_text": text,
    }


def verify_telebirr_sms(sms_text, expected_amount, expected_receiver=None):
    """Verify SMS details against the official Telebirr receipt page."""
    extracted = extract_telebirr_sms(sms_text)
    reference = extracted["reference"]

    if not reference:
        return {
            "ok": False,
            "reason": "No valid Telebirr transaction number or receipt URL was found.",
            "sms": extracted,
        }

    try:
        url, raw_html = fetch_telebirr_receipt(reference)
        receipt = parse_telebirr_receipt(reference, raw_html)
    except Exception as exc:
        return {
            "ok": False,
            "reason": f"Telebirr receipt could not be reached or parsed: {type(exc).__name__}",
            "sms": extracted,
            "error": str(exc),
        }

    receipt_amount = receipt.get("settled_amount")
    if receipt_amount is None:
        receipt_amount = receipt.get("total_paid_amount")

    checks = []
    checks.append((receipt.get("receipt_no", "").upper() == reference.upper(), "transaction number"))

    if receipt_amount is not None:
        checks.append((abs(float(receipt_amount) - float(expected_amount)) < 0.00001, "amount"))
    else:
        checks.append((False, "amount"))

    if expected_receiver and receipt.get("credited_party_name"):
        a = re.sub(r"\s+", " ", expected_receiver.strip()).upper()
        b = re.sub(r"\s+", " ", receipt["credited_party_name"].strip()).upper()
        checks.append((a == b, "receiver"))

    status = (receipt.get("transaction_status") or "").upper()
    if status:
        checks.append((status in {"COMPLETED", "SUCCESS", "PAY_SUCCESS"}, "status"))

    failed = [name for ok, name in checks if not ok]

    return {
        "ok": not failed,
        "reason": "verified" if not failed else "Mismatch: " + ", ".join(failed),
        "sms": extracted,
        "receipt": receipt,
        "receipt_url": url,
    }
...
async def start_sms_payment_verification(update, context, plan):
    user = update.effective_user
    amount = MONTHLY_PRICE_ETB if plan == "monthly" else YEARLY_PRICE_ETB
    context.user_data["awaiting_payment_sms"] = plan
    await update.message.reply_text(
        "📩 Telebirr payment verification\n\n"
        f"💎 Plan: {plan.title()} Premium\n"
        f"💰 Required amount: {amount:.2f} ETB\n\n"
        "After paying, copy the FULL Telebirr SMS from 127 and paste it here.\n\n"
        "The bot will check the transaction number and official Telebirr receipt.\n"
        "Premium will be activated only when the official receipt matches the required amount."
    )
...
async def chat(
    ...
    if payment_plan:
        ...
        amount = MONTHLY_PRICE_ETB if payment_plan == "monthly" else YEARLY_PRICE_ETB

        await update.message.reply_text(
            "🔎 Telebirr receipt is being checked...\n\n"
            "Please wait."
        )
        ...
        if not result.get("ok"):
            await update.message.reply_text(
                ...
                f"Reason: {result.get('reason', 'Invalid receipt')}\n\n"
                "Please make sure you pasted the FULL Telebirr SMS from 127 and that the payment amount is correct."
            )
            return
        ...
        expiry = set_premium(
            user.id,
            MONTHLY_DAYS if payment_plan == "monthly" else YEARLY_DAYS,
        )

        await update.message.reply_text(
            "🎉 PAYMENT VERIFIED\n\n"
            "💎 Ethio AI Premium is now ACTIVE.\n"
            f"💰 Verified amount: {amount:.2f} ETB\n"
            f"🆔 Transaction: {receipt.get('receipt_no') or result['sms'].get('reference')}\n"
            f"📅 Expires: {expiry.strftime('%Y-%m-%d %H:%M UTC')}\n\n"
