...
REFERRAL_REWARD_ETB = int(os.getenv("REFERRAL_REWARD_ETB", "0"))
...
async def create_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Handles /withdraw AMOUNT; /withdraw without amount shows instructions.
    if not update.effective_user or not update.message:
        return
    user = update.effective_user
    save_user(user)
    ...
    try:
        ...
        await update.message.reply_text("❌ Enter a valid whole ETB amount. Example: /withdraw 50")
        return
    balance = get_balance(user.id)
...
async def button_handler(
    ...
    if data == "my_balance":
        balance = get_balance(user.id)
        balance, invites, lifetime, withdrawn = get_account_financials(user.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("💸 Withdraw", callback_data="withdraw_menu")]])
        await query.message.reply_text(
            "👤 Account Info\n"
...
async def pending_withdrawals_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("""SELECT w.id,w.user_id,w.amount_etb,w.created_at,u.first_name,u.last_name,u.username
    ...
    for r in rows:
        ...
        out.append(f"🆔 Request: {r['id']}\n👤 {name}\n📱 @{r['username'] if r['username'] else 'none'}\n🆔 User ID: {r['user_id']}\n💵 Amount: {r['amount_etb']:.2f} ETB\n🕐 {r['created_at']}\n✅ /approvewithdraw {r['id']}\n❌ /rejectwithdraw {r['id']}\n")
    await update.message.reply_text('\n'.join(out))
...
async def approve_withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Usage: /approvewithdraw REQUEST_ID"); return
    try: rid=int(context.args[0])
    except ValueError:
        await update.message.reply_text("❌ Invalid request ID."); return
    conn=get_db(); cur=conn.cursor()
    cur.execute("SELECT id,user_id,amount_etb,status FROM withdrawals WHERE id=?",(rid,)); r=cur.fetchone()
    if not r:
        conn.close(); await update.message.reply_text("❌ Withdrawal request not found."); return
    if r['status']!='pending':
        conn.close(); await update.message.reply_text(f"⚠️ Request #{rid} is already {r['status']}."); return
    cur.execute("SELECT COALESCE(balance_etb,0) AS balance FROM users WHERE user_id=?",(r['user_id'],)); u=cur.fetchone()
    bal=int(u['balance']) if u else 0
    if bal < r['amount_etb']:
        conn.close(); await update.message.reply_text(f"❌ Insufficient balance. Available: {bal:.2f} ETB"); return
    cur.execute("UPDATE users SET balance_etb=COALESCE(balance_etb,0)-? WHERE user_id=?",(r['amount_etb'],r['user_id']))
    cur.execute("UPDATE withdrawals SET status='approved',reviewed_at=?,reviewed_by=? WHERE id=? AND status='pending'",(now_iso(),ADMIN_ID,rid))
    conn.commit(); conn.close()
    await update.message.reply_text(f"✅ Withdrawal #{rid} approved.\n💵 Amount: {r['amount_etb']:.2f} ETB\n👤 User ID: {r['user_id']}")
    try: await context.bot.send_message(r['user_id'],f"✅ WITHDRAWAL APPROVED\n\n💵 Amount: {r['amount_etb']:.2f} ETB\n🆔 Request: #{rid}\n\nYour withdrawal has been approved by the admin.")
    except Exception: logger.exception("Could not notify user about approved withdrawal")

async def reject_withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user or update.effective_user.id != ADMIN_ID: return
    if not context.args:
        await update.message.reply_text("Usage: /rejectwithdraw REQUEST_ID [reason]"); return
    try: rid=int(context.args[0])
    except ValueError:
        await update.message.reply_text("❌ Invalid request ID."); return
    reason=' '.join(context.args[1:]).strip() or 'No reason provided.'
    conn=get_db(); cur=conn.cursor(); cur.execute("SELECT id,user_id,amount_etb,status FROM withdrawals WHERE id=?",(rid,)); r=cur.fetchone()
    if not r:
        conn.close(); await update.message.reply_text("❌ Withdrawal request not found."); return
    if r['status']!='pending':
        conn.close(); await update.message.reply_text(f"⚠️ Request #{rid} is already {r['status']}."); return
    cur.execute("UPDATE withdrawals SET status='rejected',reviewed_at=?,reviewed_by=?,rejection_reason=? WHERE id=? AND status='pending'",(now_iso(),ADMIN_ID,reason,rid))
    conn.commit(); conn.close()
    await update.message.reply_text(f"❌ Withdrawal #{rid} rejected.\nReason: {reason}")
    try: await context.bot.send_message(r['user_id'],f"❌ WITHDRAWAL REJECTED\n\n💵 Amount: {r['amount_etb']:.2f} ETB\n🆔 Request: #{rid}\n📝 Reason: {reason}\n\nYour balance was not deducted.")
    except Exception: logger.exception("Could not notify user about rejected withdrawal")
