...
def detect_tts_language(text: str) -> str:
    """Best-effort language selection for Google TTS."""
    # Amharic Unicode block
    if any("\u1200" <= ch <= "\u137f" for ch in text):
        return "am"

    # Common Afaan Oromoo markers. Google TTS may support Oromo; if not,
    # the caller falls back to English so the user still receives audio.
    oromo_markers = (
        "akka", "maal", "ati", "ani", "jette", "jedhe", "keessa",
        "dha", "miti", "eessa", "maaliif", "akkam", "nagaa", "galatoomi",
        "barbaada", "hojjedhu", "sirriitti"
    )
    lower = text.lower()
    if any(word in lower.split() for word in oromo_markers):
        return "om"

    return "en"


async def make_tts_file(text: str) -> str:
    """Create an MP3 voice response and return its temporary path."""
    ...
    try:
        try:
            await asyncio.to_thread(
                gTTS(text=text, lang=lang, slow=False).save,
                path,
            )
        except Exception:
            # If Oromo TTS is unavailable in the installed Google TTS
            # language set, fall back to English instead of failing the reply.
            if lang != "en":
                await asyncio.to_thread(
...
async def voice_chat(
    ...
    """Receive a Telegram voice message, ask Gemini, then return text + voice."""
    if not update.message or not update.effective_user:
        return
    ...
    try:
        ...
        # Gemini understands audio files. Upload the Telegram voice recording
        # and ask it to transcribe + answer in the same language.
...
Listen to the user's voice message and understand what they are asking.
Answer the user's request directly.

Language rules:
- If the user speaks Afaan Oromoo, answer in Afaan Oromoo.
- If the user speaks English, answer in English.
- If the user speaks Amharic, answer in Amharic.
- If the user mixes languages, respond naturally in the same style.

Do not explain that you are transcribing the audio unless necessary.
...
"""
        ...
        if not answer:
            await update.message.reply_text(
                "⚠️ I could not understand the voice message. Please try again."
            )
            return

        # Always send the normal text answer too.
        for i in range(0, len(answer), 4000):
            await update.message.reply_text(answer[i:i + 4000])

        # Then send a spoken answer.
        try:
            tts_path = await make_tts_file(answer)
            with open(tts_path, "rb") as audio_file:
                await update.message.reply_audio(
                    audio=audio_file,
                    title="Ethio AI Voice Reply",
                    performer="Ethio AI",
                )
        except Exception:
            logger.exception("VOICE TTS ERROR")
            await update.message.reply_text(
                "🔊 I sent the answer as text, but voice playback could not be generated."
            )

    except Exception as e:
        logger.exception("VOICE CHAT ERROR")
        await update.message.reply_text(
            "⚠️ I could not process your voice message.\n\n"
            f"{type(e).__name__}: {e}"
        )
