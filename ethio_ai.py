...
def init_database():
    ...
    try:
        cursor.execute(
            "ALTER TABLE payment_requests ADD COLUMN confirmation_sms TEXT"
        )
    except sqlite3.OperationalError:
        pass
...
async def show_payment_instructions(
    ...
    # Amharic Telebirr payment instructions requested by the owner.
    if plan == "monthly":
    ...
    await query.edit_message_text(
        "✅ የትዕዛዝዎ ማጠቃለያ\n\n"
        "ምርት፡ 💎 ETHIO AI PREMIUM\n"
        f"የእቅድ አይነት፡ {plan_label}\n"
        f"ዋጋ፡ {amount:,.2f} ETB\n"
        "የክፍያ ዘዴ፡ Telebirr\n\n"
        ...
        "⚠️ የክፍያ መመሪያዎች፦\n\n"
        f"1️⃣ በትክክል `{amount:,.2f}` ETB ከላይ ወደተጠቀሰው የTelebirr አካውንት ይላኩ።\n\n"
        "2️⃣ ክፍያውን ከፈጸሙ በኋላ ከTelebirr (127) የሚደርስዎትን የSMS ማረጋገጫ ይመልከቱ።\n\n"
        "3️⃣ ⚠️ አስፈላጊ፦ ከTelebirr የደረሰዎትን ሙሉ የክፍያ ማረጋገጫ SMS ኮፒ ያድርጉ።\n\n"
        "4️⃣ የኮፒ ያደረጉትን ሙሉ SMS መልዕክት እዚህ ይላኩ።\n\n"
        "5️⃣ አስተዳዳሪው ክፍያውን ካረጋገጠ በኋላ 💎 Premium አባልነትዎ ይነቃል።\n\n"
        "⏳ ይህ የክፍያ ጥያቄ በ30 ደቂቃ ውስጥ ያበቃል።\n\n"
        "⚠️ Premium የሚነቃው ክፍያው በአስተዳዳሪ ከተረጋገጠ ብቻ ነው።",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def button_handler(
    ...
    if data.startswith("upload_payment:"):
        ...
        await query.message.reply_text(
            ...
            f"Plan: {plan.title()}\n"
            f"Amount: {amount:,.2f} ETB\n\n"
            "Copy the complete Telebirr confirmation SMS and paste it here.\n\n"
    ...
    if data.startswith("approve_payment:"):
        ...
        if query.message.photo:
            ...
            await query.edit_message_text(text=approved_text, reply_markup=None)
    ...
    if data.startswith("reject_payment:"):
        ...
        if query.message.photo:
            ...
            await query.edit_message_text(text=rejected_text, reply_markup=None)
...
async def payment_confirmation_handler(
    ...
    """Receive the copied Telebirr confirmation SMS as text."""
    ...
    plan = context.user_data.get("awaiting_payment_sms")
    if not plan:
        return
    ...
    if not sms_text:
        await update.message.reply_text(
            "📩 Please copy the complete Telebirr confirmation SMS and send it here."
        )
        return
    ...
    cur.execute(
        """
        INSERT INTO payment_requests
        (user_id, plan, amount_etb, screenshot_file_id, confirmation_sms, status, created_at)
        VALUES (?, ?, ?, ?, ?, 'pending', ?)
        """,
        (user.id, plan, amount, "", sms_text, now_iso()),
    )
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"{sms_text}\n\n"
                "👇 Verify the SMS and choose:"
            ),
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
    except Exception:
        logger.exception("Could not send payment SMS to admin.")
        await update.message.reply_text(
            "⚠️ Your payment SMS was saved, but I could not notify the admin automatically."
        )
...
async def chat(
    ...
    if context.user_data.get("awaiting_payment_sms"):
...
async def image_chat(
    ...
    if context.user_data.get("awaiting_payment_sms"):
        await update.message.reply_text(
            "📩 Please send the complete Telebirr confirmation SMS as text (Copy → Paste → Send)."
        )
        return
...
def main():
    ...
    # Text messages
    # In menu-only mode, typed messages are not sent to Gemini.
    app.add_handler(
        MessageHandler(
            filters.TEXT & ~filters.COMMAND,
            chat if TEXT_CHAT_ENABLED else text_chat_disabled,
        )
    )
