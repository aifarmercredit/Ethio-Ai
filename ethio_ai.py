...
async def show_payment_instructions(
    ...
    """Show the Ethio AI Telebirr order/payment instructions.

    The message follows the requested order-summary style. It does NOT ask
    for a screenshot. The user pays the displayed amount and sends the full
    Telebirr confirmation SMS as text for admin verification.
    """
    user = query.from_user
    ...
    plan_title = (
        "Monthly Premium" if plan == "monthly" else "Yearly Premium"
    )
    ...
    payment_text = (
        "✅ Your Order Summary\n"
        f"Product: {plan_title}\n"
        f"Player ID: {player_id}\n"
        f"Player Name: {player_name}\n"
        f"Price: {amount_text} ETB\n"
        "Bank: Telebirr\n\n"
        f"Please send exactly {amount_text} ETB to:\n"
        f"•   Name: {TELEBIRR_NAME}\n"
        f"•   Account: {TELEBIRR_PHONE}\n\n"
        ...
        "3. ⚠️ ማሳሰቢያ፡ ከ Telebirr የተላከሎትን የክፍያ SMS መልእክት ሙሉውን (Copy) ያድርጉ።\n"
        "4. ኮፒ ያደረጉትን ሙሉ መልእክት እዚህ በመላክ የክፍያ ማረጋገጫዎን ያስገቡ።\n\n"
